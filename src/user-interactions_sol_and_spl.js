 ...
 */
async function createRandomnessAccount(connection, keypair) {
...
async function getPaymentDetails(user, currency, config, connection) {
  let paymentMint;
  let paymentAmount;
  let paymentType;
 ...
 */
export async function pullGacha(userPrivateKey, currency, network) {
  ...
  const paymentConfigExists = gachaState.paymentConfigs.some(
    (config) => config.toString() === paymentConfigPDA.toString()
  );
