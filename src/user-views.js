...
async function viewGachaState(network) {
  console.log(`\n📊 Viewing Gacha State...`);
...
async function viewPlayerState(userPublicKey, nonce, network = "mainnet") {
  ...
  try {
    if (nonce !== undefined) {
      ...
      try {
        ...
        playerState = await client.getPlayerState(user, new BN(nonce));
      ...
      for (let i = 0; i < totalPulls; i++) {
        const playerStatePDA = client.findPlayerStatePDA(user, new BN(i));

        try {
          // Use the gacha client's getPlayerState method
          playerState = await client.getPlayerState(user, new BN(i));
...
async function decryptPrize(encryptedKey, encryptionKey) {
  console.log(`\n🔓 Decrypting Prize...`);
