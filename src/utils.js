...
import gachaIDL from "../config/gacha_machine.json" with { type: 'json' };
// import { GachaMachine } from "../target/types/gacha_machine"; // Give it to Client
import { GachaClient } from "./gacha-client.js";

import sharedConfig from "../config/shared.json" with { type: 'json' };
 ...
 */
export function keypairFromPrivateKey(privateKeyString) {
  try {
...
export const getConfigurationFromNetwork = (network) => {
...
export function anchorProviderFromWallet(connection, user) {
  const wallet = {
...
export function createAnchorProgramAndClient(network, user) {
  ...
  const program = new anchor.Program(gachaIDL, provider);
  const client = new GachaClient(program, provider);
 ...
 */
export async function validateSolBalance(connection, user, requiredAmount) {
